...
from scipy.misc import imresizee


def img_preprocess(img, img_mean=np.float32([104, 117, 123]), img_std=np.float32([1, 1, 1])):
    '''convert to Caffe's input image layout'''
    return (np.float32(np.transpose(img, (2, 0, 1))[::-1]) - np.reshape(img_mean, (3, 1, 1)))/np.reshape(img_std, (3, 1, 1))
...
def normalise_img(img, clip=0):
    '''Normalize the image.
    Map the minimum pixel to 0+clip; map the maximum pixel to 255-clip.
    Convert the pixels to be int
    '''
    
    if img.max() > img.min():
        img = img * ((255.0 - 2 * clip)/(img.max()-img.min()))
        img = img - img.min() + clip
    else:
        img = clip
...
def switch_Model_func(caffe_root, img_mean_root, modelType):
    switcher = {
             'VGG_ILSVRC_19_layers': [caffe_root + modelType + '/VGG_ILSVRC_19_layers_sgh.prototxt', \
             caffe_root + modelType + '/VGG_ILSVRC_19_layers.caffemodel', np.array([103.939, 116.779, 123.68],dtype = 'float32'),\
             np.array([1.0, 1.0, 1.0],dtype = 'float32')],
             'VGG_ILSVRC_16_layers_2014': [caffe_root + modelType + '/VGG_ILSVRC_16_layers.prototxt', \
             caffe_root + modelType + '/VGG_ILSVRC_16_layers.caffemodel', np.array([103.939, 116.779, 123.68],dtype = 'float32'),\
             np.array([1.0, 1.0, 1.0],dtype = 'float32')],
             'ResNet-50_ILSVRC2015': [caffe_root + modelType + '/ResNet-50-deploy.prototxt', \
             caffe_root + modelType + '/ResNet-50-model.caffemodel', img_mean_root + '/ilsvrc_2015_mean.npy',\
             np.array([1.0, 1.0, 1.0],dtype = 'float32')],
             'ResNet-101_ILSVRC2015': [caffe_root + modelType + '/ResNet-101-deploy.prototxt', \
             caffe_root + modelType + '/ResNet-101-model.caffemodel', img_mean_root + '/ilsvrc_2015_mean.npy',\
             np.array([1.0, 1.0, 1.0],dtype = 'float32')],
             'ResNet-152_ILSVRC2015': [caffe_root + modelType + '/ResNet-152-deploy.prototxt', \
             caffe_root + modelType + '/ResNet-152-model.caffemodel', img_mean_root + '/ilsvrc_2015_mean.npy',\
             np.array([1.0, 1.0, 1.0],dtype = 'float32')],
              'bvlc_googlenet': [caffe_root + modelType + '/bvlc_googlenet.prototxt', \
             caffe_root + modelType + '/bvlc_googlenet.caffemodel', img_mean_root + '/ilsvrc_2012_mean.npy',\
             np.array([1.0, 1.0, 1.0],dtype = 'float32')],
              'bvlc_alexnet': [caffe_root + modelType + '/bvlc_alexnet_h.prototxt', \
             caffe_root + modelType + '/bvlc_alexnet.caffemodel', img_mean_root + '/ilsvrc_2012_mean.npy',\
             np.array([1.0, 1.0, 1.0],dtype = 'float32')],
              'bvlc_reference_caffenet': [caffe_root + modelType + '/bvlc_reference_caffenet_sgh.prototxt', \
             caffe_root + modelType + '/bvlc_reference_caffenet.caffemodel', img_mean_root + '/ilsvrc_2012_mean.npy',\
             np.array([1.0, 1.0, 1.0],dtype = 'float32')],
             'vgg-s': [caffe_root + modelType + '/vgg_s_rename_layer.prototxt', \
             caffe_root + modelType + '/VGG_CNN_S.caffemodel', img_mean_root + '/ilsvrc_2012_mean.npy',\
             np.array([1.0, 1.0, 1.0],dtype = 'float32')],
             'vgg-f': [caffe_root + modelType + '/vgg_f_rename_layer.prototxt', \
             caffe_root + modelType + '/VGG_CNN_F.caffemodel', img_mean_root + '/ilsvrc_2012_mean.npy',\
             np.array([1.0, 1.0, 1.0],dtype = 'float32')],
             'vgg-m': [caffe_root + modelType + '/vgg-m_rename_layer.prototxt', \
             caffe_root + modelType + '/VGG_CNN_M.caffemodel', img_mean_root + '/ilsvrc_2012_mean.npy',\
             np.array([1.0, 1.0, 1.0],dtype = 'float32')],
             'inception-v3': [caffe_root + modelType + '/inception-v3_rename_layer.prototxt', \
             caffe_root + modelType + '/inception-v3.caffemodel', np.array([128.0, 128.0, 128.0],dtype = 'float32'),\
             np.array([128.0, 128.0, 128.0],dtype = 'float32')],
            'inception-v4': [caffe_root + modelType + '/deploy_inception-v4_rename_layer.prototxt', \
             caffe_root + modelType + '/inception-v4.caffemodel', np.array([128.0, 128.0, 128.0],dtype = 'float32'),\
             np.array([128.0, 128.0, 128.0],dtype = 'float32')],
            'densenet121': [caffe_root + modelType + '/DenseNet_121.prototxt', \
             caffe_root + modelType + '/DenseNet_121.caffemodel', np.array([103.94, 116.78, 123.68],dtype = 'float32'),\
             np.array([1.0, 1.0, 1.0],dtype = 'float32')],
            'densenet161': [caffe_root + modelType + '/DenseNet_161.prototxt', \
             caffe_root + modelType + '/DenseNet_161.caffemodel', np.array([103.94, 116.78, 123.68],dtype = 'float32'),\
             np.array([1.0, 1.0, 1.0],dtype = 'float32')],
            'densenet169': [caffe_root + modelType + '/DenseNet_169.prototxt', \
             caffe_root + modelType + '/DenseNet_169.caffemodel', np.array([103.94, 116.78, 123.68],dtype = 'float32'),\
             np.array([1.0, 1.0, 1.0],dtype = 'float32')],
            'densenet201': [caffe_root + modelType + '/DenseNet_201.prototxt', \
             caffe_root + modelType + '/DenseNet_201.caffemodel', np.array([103.94, 116.78, 123.68],dtype = 'float32'),\
             np.array([1.0, 1.0, 1.0],dtype = 'float32')]
            }
    return switcher.get(modelType, "Invalid modelType")
    
def switch_trainNets(folder):
    switcher = {
             0: ['VGG_ILSVRC_16_layers_2014', 'ResNet-101_ILSVRC2015', 'ResNet-152_ILSVRC2015', 'bvlc_googlenet'],
             1: ['ResNet-50_ILSVRC2015', 'ResNet-101_ILSVRC2015', 'ResNet-152_ILSVRC2015','bvlc_googlenet'],
             2: ['VGG_ILSVRC_16_layers_2014', 'ResNet-50_ILSVRC2015', 'ResNet-101_ILSVRC2015', 'ResNet-152_ILSVRC2015'],
             3: ['VGG_ILSVRC_16_layers_2014', 'ResNet-50_ILSVRC2015', 'bvlc_googlenet', 'ResNet-152_ILSVRC2015'],
             4: ['VGG_ILSVRC_16_layers_2014', 'ResNet-50_ILSVRC2015', 'bvlc_googlenet', 'ResNet-101_ILSVRC2015']
            }
    return switcher.get(folder, "Invalid modelType")
